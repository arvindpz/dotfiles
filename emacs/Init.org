...
M-x which-key-show-major-mode gives a list of keybindings that we can perfrom in the current major mode
Which key has many keybindings in general. Explore them if you want to become proficient with emacs

This might be helpful in debugging init.el
https://emacs.stackexchange.com/questions/27001/debugging-some-elisp-end-of-file-during-parsing-error-in-init-el
...
** TODO Try using emacs with server and client way
- Emacs is opening fast when using client-server way; Referred to [[https://briansunter.com/blog/emacs-daemon-macos][blogpost]]
- However, The epdi iruka message is not coming up; This needs to be fixed
- Give proper names to my-frame methods
  ...
  (use-package evil
    ...
    (add-hook 'pdf-view-mode-hook
              (lambda()
                ...
                (set (make-local-variable 'pdf-view-display-size) 'fit-page))
              )
    )
...
** Font Configuration
#+begin_src emacs-lisp
  (defun my-frame-config1 (&optional frame)
    (with-selected-frame (or frame (selected-frame))
      ;; Set font
  ...
  (add-hook 'after-make-frame-functions 'my-frame-config1)
  (add-hook 'after-init-hook 'my-frame-config1)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun my-frame-config4 (&optional frame)
    (with-selected-frame (or frame (selected-frame))
      ;; do things to FRAME
      ;; (use-package nerd-icons)                  ; Icons in Modeline
  ...
  (add-hook 'after-make-frame-functions 'my-frame-config4)
  (add-hook 'after-init-hook 'my-frame-config4)
#+end_src
...
#+begin_src emacs-lisp
  (defun my-frame-config2 (&optional frame)
    (with-selected-frame (or frame (selected-frame))
      ;; do things to FRAME
  ...
  (add-hook 'after-make-frame-functions 'my-frame-config2)
  (add-hook 'after-init-hook 'my-frame-config2)
#+end_src
  ...
  (use-package org
    ...
    (org-agenda-files
     '("~/playground/Org-Files/Tasks.org"
       ...
       "~/playground/Org-Files/Health.org"
    ...
    (org-refile-targets
     '(("Archive.org" :maxlevel . 1)
       ...
       ;; Use the files that you visit as refile targets
       ;; https://lists.gnu.org/archive/html/emacs-orgmode/2009-01/msg00373.html
       (nil :maxlevel . 8)
       ))
...
**** TODO Go through Georges answer in below for refiling withing the current buffer
https://stackoverflow.com/questions/7509463/how-to-move-a-subtree-to-another-subtree-in-org-mode-emacs

**** TODO Check out Artist mode for UML diagrams
*** Org-Agenda
...
**** Java
https://www.youtube.com/watch?v=Yah69AfYP34
**** HTML and CSS
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (defun my-frameconfig6 (&optional frame)
    (with-selected-frame (or frame (selected-frame))
      ;; do things to FRAME
      (use-package winum
        :config
  ...
  (add-hook 'after-make-frame-functions 'my-frameconfig6)
  (add-hook 'after-init-hook 'my-frameconfig6)
#+end_src
...
** Hi Da
#+begin_src emacs-lisp
  (defun my-frame-config5 (&optional frame)
    (with-selected-frame (or (selected-frame) frame)
      (with-eval-after-load 'auto-package-update
        (message "Hi da, Epdi iruka?!"))
      ))

  (add-hook 'after-make-frame-functions 'my-frame-config5)
  (add-hook 'after-init-hook 'my-frame-config5)
#+end_src
